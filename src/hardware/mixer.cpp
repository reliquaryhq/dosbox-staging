...
MixerChannel * MIXER_AddChannel(MIXER_Handler handler, Bitu freq, const char * name) {
	...
	LOG_MSG("MIXER: %s channel at %u Hz, %s output rate", name, chan_rate,
	        chan_rate == mix_rate ? "matching"
	                              : chan_rate > mix_rate ? "downsampling to"
	                                                     : "upsampling to");
	return chan;
}
...
void MIXER_Init(Section* sec) {
	...
	if (mixer.nosound) {
		...
		if (obtained_freq != mixer.freq) {
			LOG_MSG("MIXER: SDL changed the playback rate from %d to %d Hz",
		...
		LOG_MSG("MIXER: Negotiated a %d-Hz output rate and %u-byte blocksize",
